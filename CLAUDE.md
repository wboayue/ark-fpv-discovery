...
- **[`src/sensors.rs`](src/sensors.rs) is the sensor *role layer*** — the contract every sensor of a role produces, kept separate from the concrete chip. It holds (a) the **contract types**: sample structs (`ImuSample`/`BaroSample`/`MagSample`) and the *logical* config enums (`ImuOdr`/`BaroOdr`/`Oversampling`/`MagOdr`, `ConfigError`); and (b) the **role traits** `Imu`/`Baro`/`Mag` plus horizontal `Identify`/`SoftReset` (definition-only today, `#[allow(dead_code)]`; the goal is to rewrite `fusion` against `impl Imu/Baro/Mag` so the concrete chip is swappable). **A logical config enum names a rate/quantity, not a register value** — the chip-specific encoding stays *private to the driver* as `odr_reg`/`osr_reg` (datasheet-cited there), so a second IMU can satisfy the same `ImuOdr` with a different encoding. New sensor of an existing role → impl the role trait + add an `*_reg` mapping; genuinely new role → add a contract type + role trait here. Don't put register codes on the lifted enums.
