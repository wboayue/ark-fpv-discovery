...
- **Relative altitude**: fixed `P0_REFERENCE = 1013.25` hPa; on the first valid baro sample `Fusion::update` calls the estimator's `reset()` so altitude starts ≈0 "here." Vertical velocity is a derivative, unaffected by the P0 offset.
...
- **Axis/convention caveat**: `Convention::Nwu` is the default; `Fusion::to_body_frame` is the single place to apply any board-mount remap/sign-flip and is currently identity. If on hardware pitch responds to roll, or yaw spins the wrong way, fix the mapping there. Rates/gains live in [`src/config.rs`](src/config.rs) under `// --- Fusion ---`.
- Sanity check on hardware: level at rest → roll/pitch ≈ 0 after ~1–2 s convergence, alt ≈ 0.00 m, vz ≈ 0. A steadily growing vz ⇒ dt error or earth-accel sign/scale bug (at rest `earth_acceleration().z*GRAVITY` should be ≈0, **not** ≈9.81). Bench yaw may be biased by hard-iron even when correct — judge yaw by *tracking direction*, not absolute heading.
